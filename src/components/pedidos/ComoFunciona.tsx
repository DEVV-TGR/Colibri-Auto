...
export function ComoFunciona({
  ...
  return (
    <aside className="rounded-2xl border border-line/60 bg-surface/60 p-8">
      <p className="text-xs uppercase tracking-[0.2em] text-laranja-deep">{titulo}</p>
      ...
      <ol className="mt-6 space-y-6">
        {passos.map((p, i) => (
          <li key={p.titulo} className="flex gap-4">
            <span
              aria-hidden
              className="font-display shrink-0 text-lg leading-none text-laranja-deep/60"
            >
              {i + 1}
            </span>
            <div>
      ...
      <div className="mt-4 space-y-2 text-sm">
        <a
          ...
          className="block text-ink transition-colors hover:text-laranja-deep"
        >
          {stand.telemovel}
        </a>
        <a
